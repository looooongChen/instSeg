...
def image_resize_np(images, sz, method='bilinear'):
    ...
    for img in images:
        if method == 'nearest':
            resized.append(cv2.resize(img, sz, interpolation=cv2.INTER_NEAREST))
        else:
...
def edt_np(label, normalize=True):
    '''
    Inputs: B x H x W x 1
    '''
    label = np.squeeze(label, axis=-1).astype(np.uint16)
    dts = []
    pbar = tqdm(label, desc='Distance transform:', ncols=100)
    for l in pbar:
        l_dil = cv2.dilate(l, np.ones((3,3), dtype=np.uint16),iterations = 1)
        l_ero = cv2.erode(l, np.ones((3,3), dtype=np.uint16),iterations = 1)
