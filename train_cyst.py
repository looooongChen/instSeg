...
X = sorted(glob.glob('./cysts/image/*.tif'))
Y = sorted(glob.glob('./cysts/ground_truth/*.png'))
...
model = instSegV2.InstSeg(config=config, base_dir='./', run_name='all')
# model.train(train_data, batch_size=4, epochs=500)
...
plt.subplot(1,2,1)
plt.imshow(pred['embedding'][0,:,:,0:3]*m)
plt.subplot(1,2,2)
